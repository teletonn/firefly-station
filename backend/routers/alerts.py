...
async def get_alert_rules(
    ...
    try:
        ...
        for row in rows:
            ...
            # Parse JSON fields
            try:
                rule['conditions'] = json.loads(rule['conditions']) if rule.get('conditions') else {}
                rule['target_groups'] = json.loads(rule['target_groups']) if rule.get('target_groups') else []
                rule['escalation_rules'] = json.loads(rule['escalation_rules']) if rule.get('escalation_rules') else {}
            except json.JSONDecodeError as e:
                logger.error(f"Error parsing JSON for rule {rule.get('id')}: {e}")
                continue
            rules.append(rule)
...
async def create_alert_rule(
    ...
    try:
        ...
        database.log_audit(
            ...
            details=f"Created alert rule '{name}'"
        )
...
async def update_alert_rule(
    ...
    try:
        ...
        if target_groups is not None:
            ...
            values.append(json.dumps(target_groups))
        if escalation_rules is not None:
            updates.append("escalation_rules = ?")
            values.append(json.dumps(escalation_rules))
...
async def delete_alert_rule(
    ...
    try:
        ...
        # Check if rule exists
        cursor.execute('SELECT name FROM alert_rules WHERE id = ?', (rule_id,))
        ...
        database.log_audit(
            ...
            resource="alert_rule",
            resource_id=str(rule_id),
            details=f"Deleted alert rule '{rule['name']}'"
        )
