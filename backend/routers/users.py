...
async def create_user(
    ...
    """Create a new user."""
    print(f"DEBUG: POST /api/users/ called by user {current_user.get('username')}")
    if not auth.check_permission(current_user, "users"):
        print(f"DEBUG: Permission denied for user {current_user.get('username')} role {current_user.get('role')}")
...
async def get_users(
    ...
    print(f"DEBUG: GET /api/users/ called by user {current_user.get('username')}")
        ...
        for user_data in users_data:
            ...
            try:
                ...
                user_info = {
                    ...
                    "deviceMetrics": {
                        "batteryLevel": int(user_data.get("battery_level", 0)) if user_data.get("battery_level") else None
                    }
                }
...
async def export_users(
    ...
    if group_id:
        ...
        users = database.get_all_users(limit=10000)  # Large limit for export
    ...
    if format == "csv":
        ...
        for user in users:
            writer.writerow({
                ...
                "device_status": user.get("device_status", "")
            })
